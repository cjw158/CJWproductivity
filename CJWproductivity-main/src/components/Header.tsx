...
export const Header = memo(function Header({ activeTab, onTabChange, onOpenSettings }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      >
        <TabButton active={activeTab === "tasks"} onClick={handleTasksClick} icon={CheckSquare} label={t("nav.tasks")} isDark={isDark} />
        <TabButton active={activeTab === "plans"} onClick={handlePlansClick} icon={Target} label={t("nav.plans")} isDark={isDark} />
        <TabButton active={activeTab === "notes"} onClick={handleNotesClick} icon={Book} label={t("nav.notes")} isDark={isDark} />
      </nav>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          title={islandActive ? t("settings.island.enabled") : t("settings.categories.island")}
        >
        ...
        <div className={cn(
          ...
          {currentTime.toLocaleTimeString(language === "zh-CN" ? "zh-CN" : language === "ja-JP" ? "ja-JP" : "en-US", { hour: "2-digit", minute: "2-digit" })}
        </div>
      </div>
