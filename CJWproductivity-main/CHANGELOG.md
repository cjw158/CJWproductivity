...
### 2024-12-05 - 性能优化与稳定性增强

#### 🟡 性能优化 (Performance)

##### 1. React Query 缓存策略优化

**问题：** 笔记列表每3秒轮询一次，造成不必要的网络请求和性能开销。

**优化方案：**
- 文件夹查询：`staleTime` 设为5分钟（文件夹很少变化）
- 笔记查询：`staleTime` 设为10秒，轮询间隔从3秒改为30秒
- 添加乐观更新，CRUD操作立即反映在UI中，无需等待服务器响应

**效果：** 减少约90%的不必要请求，UI响应更快。

##### 2. 数据库连接优化

**优化方案：**
- 添加 `initPromise` 防止并发初始化
- 新增笔记表索引：`idx_notes_updated`（更新时间排序）和 `idx_notes_pinned`（置顶筛选）
- 使用循环批量创建索引，代码更整洁

##### 3. TaskSelectionContext 优化

**优化方案：**
- 使用 `useMemo` 缓存派生状态 `isSelecting`
- 使用 `useMemo` 缓存整个 context value，避免不必要的重渲染
- 状态更新前检查是否真的需要更新（如已清空时不再创建新Set）
- 新增 `isTaskSelected` 辅助函数，便于批量检查
