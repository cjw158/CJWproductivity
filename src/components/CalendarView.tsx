...
export const CalendarView = memo(function CalendarView() {
  ...
  const getWeekdayName = (d: number) => ["周日", "周一", "周二", "周三", "周四", "周五", "周六"][d];
    ...
    <div className="h-full flex gap-4 p-4">
      ...
      <div className={cn("w-[380px] flex-shrink-0 rounded-2xl border p-4 flex flex-col", isDark ? "bg-white/[0.02] border-white/10" : "bg-white border-gray-200")}>
        <div className="flex items-center justify-between mb-4">
          <h2 className={cn("text-lg font-bold", isDark ? "text-white" : "text-gray-900")}>{year}年{month + 1}月</h2>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isToday(selectedDate) && (
                <span className={cn("text-[10px] px-1.5 py-0.5 rounded border", isDark ? "border-[var(--neon-cyan)]/50 text-[var(--neon-cyan)]" : "border-blue-400 text-blue-500")}>今天</span>
              )}
            </div>
            <p className={cn("text-xs mt-0.5", isDark ? "text-white/40" : "text-gray-500")}>
              {dayTasks.length} 个任务，{dayTasks.filter(t => t.status !== "DONE").length} 个待完成
            </p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
          {dayTasks.length === 0 ? (
            <div className={cn("text-center py-16", isDark ? "text-white/30" : "text-gray-400")}>
              ...
              <p className="text-sm mb-3">今日无任务</p>
              <button onClick={openQuickCapture} className={cn("text-sm px-4 py-2 rounded-lg border", isDark ? "border-white/10 hover:bg-white/5 text-white/60" : "border-gray-200 hover:bg-gray-50 text-gray-600")}>
                <Plus className="w-4 h-4 inline mr-1" />快速添加
              </button>
            </div>
          ) : (
            <AnimatePresence>
